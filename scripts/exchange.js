...
const {utils} = ethers;
const {splitSignature, formatEther} = utils;
...
async function main() {
  ...
  const TransferProxyForDeprecated = await ethers.getContractFactory(
    'src/marketplace/transferproxy/TransferProxyForDeprecated.sol:TransferProxyForDeprecated'
  );
  ...
  const ERC20TransferProxy = await ethers.getContractFactory(
    'src/marketplace/transferproxy/ERC20TransferProxy.sol:ERC20TransferProxy'
  );
  ...
  const ExchangeOrdersHolderV1 = await ethers.getContractFactory(
    'src/marketplace/ExchangeOrdersHolderV1.sol:ExchangeOrdersHolderV1'
  );
  ...
  const toSignForOrder = ethers.utils
    .keccak256(ethers.utils.defaultAbiCoder.encode([exchangeOrderType], [order]))
    .slice(2);

  const signedMessageForSell = await seller.signMessage(toSignForOrder);
  const sellSig = splitSignature(signedMessageForSell);

  const toSignForOrderBuy = ethers.utils
    .keccak256(
  ...
  console.log('Balance Of Seller ', formatEther(await deployedEMUSK.balanceOf(seller.address)));
  console.log('Balance Of Buyer ', formatEther(await deployedEMUSK.balanceOf(buyer.address)));
